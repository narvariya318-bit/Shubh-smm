  ...
  if (state.db) {
    ...
    onAuthStateChanged(state.auth, (user) => {
      ...
      if (user) {
        ...
        const currentVisitId = sessionStorage.getItem('smm_current_visit_id');
...
function renderHeaderFooter() {
  ...
  logoTextEls.forEach(el => {
    const rawTitle = state.settings.title || "Shubh SMM Panel";
    if (rawTitle.toLowerCase().includes('shubh')) {
      el.innerHTML = `SHUBH <span class="bg-gradient-to-r from-indigo-600 via-purple-600 to-emerald-500 bg-clip-text text-transparent">SMM</span> PANEL`;
    } else if (rawTitle.includes('LENS')) {
      el.innerHTML = `LENS <span class="bg-gradient-to-r from-indigo-600 via-purple-600 to-emerald-500 bg-clip-text text-transparent">RA</span> SMM`;
    } else {
  ...
  if (headerLogoContainer) {
    if (state.settings.logoUrl && state.settings.logoUrl.trim() !== '') {
      headerLogoContainer.innerHTML = `<img src="${state.settings.logoUrl}" alt="SMM Logo" class="w-10 h-10 object-contain rounded-[1.1rem] shadow-sm">`;
    } else {
      headerLogoContainer.innerHTML = `
        <div class="w-10 h-10 rounded-[1.1rem] bg-gradient-to-tr from-indigo-600 via-purple-600 to-emerald-500 flex items-center justify-center text-white text-lg font-black shadow-md shadow-indigo-500/10 border border-white/20 group-hover:scale-105 transition-transform duration-300">
          🚀
        </div>
      `;
    }
...
async function trackVisit() {
  ...
  if (!state.db) {
    ...
    if (!sessionStorage.getItem('smm_visit_logged')) {
      localVisits++;
  ...
  try {
    const isNewSession = !sessionStorage.getItem('smm_visit_logged');
    if (isNewSession) {
      sessionStorage.setItem('smm_visit_logged', 'true');
      ...
      sessionStorage.setItem('smm_visit_start_time', Date.now().toString());
