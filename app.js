...
function navigateTo(viewName) {
  ...
  // Highlight mobile tab items if present
  document.querySelectorAll('.mobile-tab-item').forEach(btn => {
    const route = btn.getAttribute('data-route');
    if (route === viewName) {
      btn.classList.add('text-[#5B3DF5]', 'dark:text-indigo-400', 'scale-[1.03]');
      btn.classList.remove('text-slate-400', 'dark:text-slate-500');
    } else {
      btn.classList.remove('text-[#5B3DF5]', 'dark:text-indigo-400', 'scale-[1.03]');
      btn.classList.add('text-slate-400', 'dark:text-slate-500');
    }
  });

  // Handle center floating order button highlight
  const orderBtn = document.getElementById('mobile-order-btn');
  if (orderBtn) {
    if (viewName === 'order') {
      orderBtn.classList.remove('bg-[#5B3DF5]');
      orderBtn.classList.add('bg-emerald-500', 'scale-110', 'shadow-emerald-500/50');
    } else {
      orderBtn.classList.remove('bg-emerald-500', 'scale-110', 'shadow-emerald-500/50');
      orderBtn.classList.add('bg-[#5B3DF5]');
    }
  }
...
function renderHeaderFooter() {
  ...
  logoTextEls.forEach(el => {
    const rawTitle = state.settings.title || "SHUBH SMM";
...
function renderHomeView() {
  ...
  const homeDepositInput = document.getElementById('home-deposit-amount');
  if (homeDepositInput) {
    const minVal = state.settings.minDeposit || 20;
    homeDepositInput.placeholder = `Min ₹${minVal} to Deposit...`;
  }
...
function getServiceCardHtml(srv, cat) {
  ...
  const packagePrice = Math.round(((srv.minQuantity || 0) * (srv.pricePer1000 || 0)) / 1000);
  const qtyLabel = srv.description ? `Quantity: ${srv.description}` : `Quantity: ${(srv.minQuantity || 0).toLocaleString()}+`;
  ...
  if (srvNameLower.includes('youtube') || srvNameLower.includes('subs') || srvNameLower.includes('subscribers') || (srv.categoryId && srv.categoryId.includes('youtube'))) {
  ...
  } else if (srvNameLower.includes('instagram') || srvNameLower.includes('followers') || srvNameLower.includes('likes') || (srv.categoryId && srv.categoryId.includes('instagram'))) {
  ...
  if (srv.logoUrl && srv.logoUrl.trim() !== '') {
    serviceLogoHtml = `
      <div class="w-14 h-14 rounded-full border-2 ${themeBorderColor} p-0.5 flex items-center justify-center shrink-0 bg-white dark:bg-slate-950 shadow-md">
        <img src="${srv.logoUrl}" alt="${srv.name || 'SMM Service'}" class="w-full h-full rounded-full object-cover shrink-0">
      </div>
    `;
  } else if (srvNameLower.includes('blue tick') || srvNameLower.includes('verified') || (srv.id || '').includes('blue-tick') || srvNameLower.includes('blue_tick')) {
    // Blue Tick badge
    ...
    `;
  } else if (srvNameLower.includes('youtube') || srvNameLower.includes('subs') || srvNameLower.includes('subscribers') || (srv.categoryId && srv.categoryId.includes('youtube'))) {
  ...
  return `
    <div onclick="window.selectServiceAndNavigate('${srv.categoryId}', '${srv.id}')" class="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl p-4 flex items-center justify-between shadow-sm hover:scale-[1.015] hover:shadow-md hover:border-indigo-500/30 dark:hover:border-indigo-500/30 transition-all duration-300 cursor-pointer select-none group relative overflow-hidden bento-card">
      <div class="flex items-center gap-3.5 max-w-[65%]">
        ${serviceLogoHtml}
        <div class="min-w-0">
          <h4 class="text-xs sm:text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider leading-tight truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">${srv.name}</h4>
          ${lastOfferBadge}
          <p class="text-[9.5px] text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-wider mt-1.5 leading-none">${qtyLabel}</p>
          <p class="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase mt-1 leading-none flex items-center gap-1">
            ⏳ Offer Price: <span class="font-black text-rose-600 dark:text-rose-400">₹${packagePrice}</span>
          </p>
        </div>
      </div>
      <div class="flex items-center shrink-0">
        <!-- Buy button with price dynamic -->
        <button onclick="event.stopPropagation(); window.selectServiceAndNavigate('${srv.categoryId}', '${srv.id}')" class="px-4 py-2 bg-gradient-to-r ${themeButtonBg} text-white rounded-xl text-xs font-black tracking-wider shrink-0 transition-all duration-300 shadow-md hover:shadow-lg active:scale-95 flex items-center gap-1.5 border border-white/10 hover:brightness-105 uppercase">
          <span>₹${packagePrice}</span>
          <svg class="w-3 h-3 text-white group-hover:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
...
window.selectHomeDepositChip = function(amount) {
  const input = document.getElementById('home-deposit-amount');
  if (input) {
    input.value = amount;
  }
  
  // Highlight active chip
  const chips = [30, 50, 80, 120];
  chips.forEach(c => {
    const chipEl = document.getElementById(`home-chip-${c}`);
    if (chipEl) {
      if (c === amount) {
        chipEl.className = "home-deposit-chip py-2 border border-indigo-500 bg-indigo-50/50 dark:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400 rounded-2xl text-center flex flex-col items-center justify-center transition active:scale-95 cursor-pointer";
      } else {
        chipEl.className = "home-deposit-chip py-2 border border-slate-100 dark:border-slate-800 rounded-2xl bg-slate-50/50 dark:bg-slate-950/40 text-center flex flex-col items-center justify-center transition active:scale-95 cursor-pointer";
      }
    }
  });
};

window.handleHomeAddFunds = function() {
  const input = document.getElementById('home-deposit-amount');
  const minVal = state.settings.minDeposit || 20;
  const val = parseFloat(input?.value || String(minVal));
  if (isNaN(val) || val < minVal) {
    alert(`Please enter a minimum deposit amount of ₹${minVal}`);
    return;
  }
  state.depositAmount = val;
  // Also sync main deposit page
  const mainInput = document.getElementById('add-money-amount');
  if (mainInput) {
    mainInput.value = val;
  }
...
window.filterServicesByPlatform = function(platform) {
  state.selectedPlatformFilter = platform;
  
  // Update sub-tabs styles if we are in/navigating to the services view
  const tabs = ['all', 'instagram', 'youtube'];
  tabs.forEach(t => {
    const btn = document.getElementById(`platform-tab-${t}`);
    if (btn) {
      if (t === platform) {
        btn.className = "flex-1 py-1.5 text-[9px] font-black uppercase tracking-wider rounded-xl transition select-none cursor-pointer text-center text-indigo-600 dark:text-indigo-400 bg-white dark:bg-slate-900 shadow-sm border border-slate-200/40 dark:border-slate-800/40";
      } else {
        btn.className = "flex-1 py-1.5 text-[9px] font-black uppercase tracking-wider rounded-xl transition select-none cursor-pointer text-center text-slate-400 hover:text-slate-700 dark:hover:text-slate-300";
      }
    }
  });
...
function renderServicesList() {
  ...
  // Sync sub-tabs buttons style
  const platform = state.selectedPlatformFilter || 'all';
  const tabs = ['all', 'instagram', 'youtube'];
  tabs.forEach(t => {
    const btn = document.getElementById(`platform-tab-${t}`);
    if (btn) {
  ...
  let activeCategories = state.categories.filter(c => c.active);
  if (platform !== 'all') {
    activeCategories = activeCategories.filter(c => c.icon === platform);
  }

  if (activeCategories.length === 0) {
    container.innerHTML = `<p class="text-center text-slate-400 text-xs py-8 font-semibold">No services found for the selected platform.</p>`;
    return;
  }

  const renderedContent = activeCategories.map(cat => {
  ...
  if (renderedContent.trim() === '') {
    container.innerHTML = `<p class="text-center text-slate-400 text-xs py-8 font-semibold">No matching services available for "${searchQuery}".</p>`;
  } else {
...
function renderAdminDashboard() {
  ...
  container.innerHTML = `
    ...
    <div class="flex border-b border-slate-800 mb-4 overflow-x-auto gap-1 text-[10px] font-black uppercase scrollbar-none">
      ...
      <button onclick="window.switchAdminTab('admin-payments')" class="px-4 py-2.5 border-b-2 transition shrink-0 ${activeAdminTab === 'admin-payments' ? 'border-indigo-500 text-indigo-400 bg-indigo-950/20 font-extrabold' : 'border-transparent text-slate-400 hover:text-slate-200'}">
        💸 Confirm Payments
      </button>
      <button onclick="window.switchAdminTab('admin-orders')" class="px-4 py-2.5 border-b-2 transition shrink-0 ${activeAdminTab === 'admin-orders' ? 'border-indigo-500 text-indigo-400 bg-indigo-950/20 font-extrabold' : 'border-transparent text-slate-400 hover:text-slate-200'}">
        🛍️ Orders
      </button>
      <button onclick="window.switchAdminTab('admin-services')" class="px-4 py-2.5 border-b-2 transition shrink-0 ${activeAdminTab === 'admin-services' ? 'border-indigo-500 text-indigo-400 bg-indigo-950/20 font-extrabold' : 'border-transparent text-slate-400 hover:text-slate-200'}">
        📦 Services
      </button>
      <button onclick="window.switchAdminTab('admin-reviews')" class="px-4 py-2.5 border-b-2 transition shrink-0 ${activeAdminTab === 'admin-reviews' ? 'border-indigo-500 text-indigo-400 bg-indigo-950/20 font-extrabold' : 'border-transparent text-slate-400 hover:text-slate-200'}">
        ⭐ Reviews
      </button>
    ...
    <form id="admin-settings-form" class="space-y-4 bg-slate-900 border border-slate-800 rounded-2xl p-4 text-xs" onsubmit="window.saveGeneralSettings(event)">
      ...
      <div class="grid grid-cols-3 gap-3">
        <div>
          ...
          <input type="number" id="set-min-deposit" value="${state.settings.minDeposit || 20}" class="w-full px-2.5 py-2 bg-slate-950 border border-slate-850 text-emerald-400 font-extrabold rounded-lg outline-none">
        </div>
        <div>
...
window.saveGeneralSettings = async function(e) {
  ...
  const minDeposit = parseFloat(document.getElementById('set-min-deposit').value) || 20;
...
function renderAddMoneyView() {
  ...
  if (minLabel) minLabel.textContent = `MINIMUM ₹${minVal} • MAXIMUM ₹10,000`;

  const addMoneyInput = document.getElementById('add-money-amount');
  if (addMoneyInput) {
    addMoneyInput.placeholder = `Minimum ₹${minVal} Only`;
    addMoneyInput.setAttribute('min', minVal);
  }
  
  const offer1Amt = state.settings.bonusOffer1Amt || 120;
  ...
  const offer3Amt = state.settings.bonusOffer3Amt || 80;
  ...
  if (amt1) amt1.textContent = `₹${offer1Amt} Deposit`;
  if (bon1) bon1.textContent = `₹${offer1Extra} Extra Bonus`;

  const amt2 = document.getElementById('offer-amt-2');
  const bon2 = document.getElementById('offer-bonus-2');
  if (amt2) amt2.textContent = `₹${offer2Amt} Deposit`;
  if (bon2) bon2.textContent = `₹${offer2Extra} Extra Bonus`;

  const amt3 = document.getElementById('offer-amt-3');
  const bon3 = document.getElementById('offer-bonus-3');
  if (amt3) amt3.textContent = `₹${offer3Amt} Deposit`;
  if (bon3) bon3.textContent = `₹${offer3Extra} Extra Bonus`;

  const amt4 = document.getElementById('offer-amt-4');
  const bon4 = document.getElementById('offer-bonus-4');
  if (amt4) amt4.textContent = `₹${offer4Amt} Deposit`;
  if (bon4) bon4.textContent = `₹${offer4Extra} Extra Bonus`;

  // Render User Add Funds History List dynamically
  window.renderAddFundsHistoryList();
}
window.renderAddMoneyView = renderAddMoneyView;

window.selectAddMoneyOffer = function(offerIdx) {
  let amt = 0;
  if (offerIdx === 1) amt = state.settings.bonusOffer1Amt || 120;
  else if (offerIdx === 2) amt = state.settings.bonusOffer2Amt || 30;
  else if (offerIdx === 3) amt = state.settings.bonusOffer3Amt || 80;
  else if (offerIdx === 4) amt = state.settings.bonusOffer4Amt || 50;
  
  const input = document.getElementById('add-money-amount');
  if (input) input.value = amt;

  // Visual highlights for chips
  const chipIds = {
    1: 'bonus-chip-1',
    2: 'bonus-chip-2',
    3: 'bonus-chip-3',
  ...
  Object.keys(chipIds).forEach(key => {
    const chip = document.getElementById(chipIds[key]);
    if (chip) {
      if (parseInt(key) === offerIdx) {
        chip.className = "p-3.5 bg-amber-100/40 dark:bg-amber-950/30 border-2 border-amber-400 rounded-2xl text-center transition hover:scale-[1.01] active:scale-95 flex flex-col items-center justify-center gap-1.5 select-none cursor-pointer";
      } else {
        chip.className = "p-3.5 bg-amber-50/10 dark:bg-amber-950/10 border border-amber-200/50 dark:border-amber-900/20 rounded-2xl text-center transition hover:scale-[1.01] active:scale-95 flex flex-col items-center justify-center gap-1.5 select-none cursor-pointer";
      }
    }
  });
};
...
window.renderAddFundsHistoryList = function() {
  const container = document.getElementById('add-funds-history-list');
  if (!container) return;

  const userId = state.currentUser ? state.currentUser.uid : state.guestId;
  const userPayments = (state.paymentRequests || []).filter(req => req.userId === userId);

  if (userPayments.length === 0) {
    container.innerHTML = `
      <div class="text-center py-6 text-slate-400 dark:text-slate-500 text-xs font-semibold uppercase tracking-wider">
  ...
  container.innerHTML = userPayments.map(req => {
    let badgeClass = '';
    let statusText = '';
    ...
    return `
      <div class="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-150 dark:border-slate-850 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm hover:border-indigo-100 dark:hover:border-indigo-950 transition duration-150">
        <div class="space-y-1">
          <div class="flex items-center gap-2">
            <span class="text-xs font-extrabold text-slate-850 dark:text-slate-100">₹${parseFloat(req.amount).toFixed(2)}</span>
            <span class="text-[8px] font-black px-2 py-0.5 rounded-full ${badgeClass} uppercase tracking-wider">${statusText}</span>
          </div>
          <div class="text-[9px] text-slate-400 font-bold uppercase tracking-widest leading-none">
            UTR: <span class="font-mono text-slate-700 dark:text-slate-300 select-all">${req.utr}</span>
          </div>
        </div>
        <div class="text-left sm:text-right space-y-0.5 shrink-0">
          <span class="block text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider">${dateStr}</span>
          <span class="block text-[8px] font-mono text-slate-400 dark:text-slate-500">${req.id}</span>
        </div>
      </div>
    `;
  }).join('');
...
function syncPayments() {
  if (state.db) {
    const unsub = onSnapshot(collection(state.db, 'payment_requests'), (snap) => {
      ...
      // Keep User Add Funds History list updated in real-time
      if (typeof window.renderAddFundsHistoryList === 'function') {
        window.renderAddFundsHistoryList();
      }
    }, (err) => {
...
function setupUIHandlers() {
  ...
  // Also allow clicking 5 times on any .site-title text to trigger admin panel
  document.querySelectorAll('.site-title').forEach(el => {
    el.addEventListener('click', (e) => {
      // Don't interfere with normal page routing if it's nested
      triggerSecretAdmin();
    });
  });
}
